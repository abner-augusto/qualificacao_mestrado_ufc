...
A utilização da Realidade Virtual (RV) no treinamento em segurança na construção civil tem sido amplamente discutida na literatura devido ao seu potencial de oferecer um ambiente imersivo e interativo para a capacitação dos trabalhadores. Revisões sistemáticas com meta-análise consolidam essa evidência: \citeonline{scorgie2024} e \citeonline{man2024} concluem que o treinamento em RV supera a instrução tradicional em aquisição e retenção de conhecimento e em mudança de comportamento de segurança, e estudos recentes corroboram o efeito para a identificação de perigos e os cenários de queda de altura aqui priorizados \cite{guo2024,alzarrad2024}. \citeonline[p.~9-10]{akindele2024} destacam que a RV permite uma experiência de aprendizagem envolvente e realista, contribuindo para que os trabalhadores compreendam de forma mais adequada os riscos e adotem estratégias apropriadas para lidar com diferentes situações de perigo nos canteiros de obras. Essa tecnologia permite criar ambientes simulados para identificação de riscos, treinamento operacional e educação em segurança \cite{zhou2019,wang2020,akindele2024}.
...
A efetividade dessas experiências em RV foi avaliada na literatura de três maneiras principais:
...
\subsection{Síntese dos Métodos Identificados na Literatura}

A análise dos estudos revisados revela uma convergência metodológica em torno de três dimensões avaliativas: (i) \textbf{ganho de conhecimento}, medido por testes pré e pós-treinamento \cite{abotaleb2023,joshi2021}; (ii) \textbf{experiência subjetiva no ambiente virtual}, capturada por questionários de presença, usabilidade e aceitação \cite{nykanen2020,xu2021,joshi2021}; e (iii) \textbf{comportamento e desempenho observado}, registrado por métricas quantitativas dentro do ambiente RV \cite{xu2021}. Todos os estudos que adotaram grupos de controle recorreram a delineamentos experimentais ou quase-experimentais, com distribuição aleatória dos participantes \cite{abotaleb2023,nykanen2020,joshi2021}, o que é reconhecido como padrão-ouro para inferência causal em pesquisas educacionais.

A comparação entre os instrumentos utilizados evidencia que o \gls{SUS} e o \gls{SSQ} aparecem como os questionários mais recorrentes para mensurar, respectivamente, usabilidade e efeitos adversos da imersão. A sensação de presença, por sua vez, é avaliada por diferentes escalas — \gls{IPQ} \cite{schubert2001} em \citeonline{nykanen2020}, e o PQ (\textit{Presence Questionnaire}) em \citeonline{joshi2021} — indicando que ainda não há um instrumento universal consolidado para essa dimensão no contexto de treinamentos em RV para segurança do trabalho. Essa pluralidade metodológica reforça a importância de justificar explicitamente a escolha dos instrumentos e de utilizar versões validadas para o idioma do estudo.

\subsection{Instrumentos de Avaliação Adotados neste Estudo}

Com base nas convergências e lacunas identificadas na literatura, os instrumentos de avaliação desta pesquisa foram selecionados para cobrir as duas dimensões centrais dos estudos revisados — ganho de conhecimento e experiência no ambiente virtual —, priorizando escalas com validação psicométrica documentada e versões adaptadas ao contexto lusófono. A avaliação de efeitos adversos da imersão (\textit{cybersickness}) foi deliberadamente excluída do escopo deste estudo piloto, de modo a reduzir a carga de resposta dos participantes e concentrar a avaliação nas hipóteses principais da pesquisa.

\subsubsection{System Usability Scale (SUS)}

O \gls{SUS} foi proposto por \citeonline{brooke1996} como um instrumento de avaliação de usabilidade de aplicação rápida e de baixo custo, composto por dez itens em escala Likert de cinco pontos, cujo escore final varia de 0 a 100. Escores acima de 68 indicam usabilidade acima da média, e acima de 80,3 são considerados excelentes \cite{brooke2013}. Apesar de sua origem em sistemas de software corporativo, o SUS consolidou-se como referência em estudos de RV \cite{joshi2021}, sendo sensível a variações de interface mesmo em amostras pequenas \cite{brooke2013}. Para este estudo, será utilizada a versão brasileira validada por \citeonline{lourencovalentim2022}, que demonstrou equivalência semântica, idiomática, conceitual e cultural com o instrumento original, com consistência interna de $\alpha = 0{,}76$, garantindo adequação ao contexto cultural dos participantes.

\subsubsection{iGroup Presence Questionnaire (IPQ)}

O \gls{IPQ} foi desenvolvido por \citeonline{schubert2001} para mensurar a sensação de presença em ambientes virtuais a partir de três dimensões: presença espacial (\textit{Spatial Presence} — SP), que avalia a sensação de ``estar'' no ambiente virtual; envolvimento (\textit{Involvement} — INV), que mede a atenção alocada ao ambiente e o afastamento do mundo físico; e realismo percebido (\textit{Experienced Realism} — REAL), que verifica a correspondência entre o ambiente virtual e o mundo real. O instrumento é composto por 14 itens em escala de sete pontos e tem sido amplamente adotado em pesquisas de treinamento em RV \cite{nykanen2020}. A escolha do IPQ em detrimento de alternativas como o \textit{Presence Questionnaire} (PQ) deve-se à disponibilidade de uma versão validada para o português por \citeonline{vasconcelosraposo2016}, com adaptações lexicais mínimas para o português brasileiro, o que confere maior rigor à comparação dos resultados com estudos internacionais.

\subsubsection{Teste de Conhecimento (pré e pós-teste)}

O teste de conhecimento, aplicado a ambos os grupos antes e imediatamente após o treinamento, constitui a principal medida de efetividade de aprendizagem. Essa abordagem é consistente com os estudos de \citeonline{abotaleb2023} e \citeonline{joshi2021} e permite calcular o ganho normalizado de aprendizagem, indicador que controla o nível de conhecimento inicial dos participantes. As questões serão elaboradas a partir dos conteúdos abordados nos cenários virtuais — seleção de \glspl{EPI} e trabalho em altura — e validadas por especialistas em segurança do trabalho antes da aplicação.

\subsubsection{Reduced Instructional Materials Motivation Survey (RIMMS)}

A avaliação da motivação e do engajamento dos participantes será realizada por meio do \textit{Reduced Instructional Materials Motivation Survey} (RIMMS), versão reduzida do IMMS derivada do modelo ARCS de \citeonline{keller1987}. O modelo ARCS organiza os fatores motivacionais em quatro dimensões: \textbf{A}tenção (\textit{Attention}), que representa o interesse e a curiosidade despertados pelo material instrucional; \textbf{R}elevância (\textit{Relevance}), que mede a percepção de utilidade e alinhamento do conteúdo com os objetivos pessoais do aprendiz; \textbf{C}onfiança (\textit{Confidence}), que avalia a crença do participante em sua capacidade de aprender e ser bem-sucedido; e \textbf{S}atisfação (\textit{Satisfaction}), que verifica o sentimento de recompensa e realização ao final da experiência.

O RIMMS operacionaliza esse modelo em 12 itens em escala Likert de cinco pontos — três por subescala —, sendo uma versão compacta do IMMS original de 36 itens. \citeonline{loorbach2015} propuseram e validaram o RIMMS em contexto de aprendizagem autodirigida com tecnologia, confirmando sua estrutura fatorial e adequação para ambientes instrucionais digitais com menor carga de resposta para os participantes. Para este estudo, será utilizada a versão traduzida e adaptada para o português brasileiro por \citeonline{cardosojunior2020}, que demonstrou equivalência semântica, idiomática, conceitual e cultural com o instrumento original.

O RIMMS será aplicado imediatamente após o treinamento a ambos os grupos — experimental (RV) e controle (vídeo instrucional) —, permitindo comparação direta da experiência motivacional entre os dois métodos nas quatro dimensões do modelo ARCS. Essa dimensão é relevante porque, conforme \citeonline{joshi2021}, o maior engajamento gerado pelo treinamento em RV tende a se converter em melhor retenção de conhecimento a longo prazo, mesmo quando o ganho cognitivo imediato é similar ao de métodos tradicionais.
...
\textbf{Lacuna na arquitetura de software dos sistemas de RV:}
\begin{itemize}
    \item A literatura de RV para segurança descreve funções, cenários e resultados de aprendizagem, mas raramente documenta a \textit{arquitetura interna} do software --- estrutura de módulos, comunicação, manutenção e extensibilidade. \citeonline{obukhov2022} identificam que arquiteturas monolíticas em complexos de treinamento virtual geram alto acoplamento entre simulação, renderização e avaliação, encarecendo a evolução e a manutenção, e recomendam decomposição arquitetural. \citeonline{quamara2024} propõem, de forma análoga, uma arquitetura modular para sistemas de Realidade Estendida que separa entrada, gerência de cena e lógica de aplicação em componentes substituíveis, motivados pela heterogeneidade de hardware e pela frequência de atualizações de \gls{SDK}. Essa lacuna justifica tratar a arquitetura do sistema como uma contribuição de engenharia desta pesquisa, e não como mero detalhe de implementação.
\end{itemize}
