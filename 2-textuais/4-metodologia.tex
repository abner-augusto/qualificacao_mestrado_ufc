...
Os cenários de treinamento serão definidos a partir de consultas com especialistas em segurança do trabalho e representantes do setor da construção civil. Esta fase contempla dois cenários principais, alinhados às diretrizes da NR-18 e NR-35:
...
\begin{itemize}
    ...
    \item \textbf{Questionário de Motivação e Engajamento:} Aplicado após o treinamento, avalia o nível de motivação e engajamento dos participantes durante a experiência, permitindo comparação direta entre os métodos.
\end{itemize}

\subsection{Instrumentos aplicados exclusivamente ao grupo experimental}

\begin{itemize}
    \item \textbf{\textit{System Usability Scale} (SUS):} Instrumento padronizado para mensurar a usabilidade e aceitação do sistema pelos usuários. Será utilizada a versão validada para o português brasileiro.
    \item \textbf{\textit{iGroup Presence Questionnaire} (IPQ):} Avalia a sensação de presença e imersão no ambiente virtual. Será utilizada a versão portuguesa validada por \citeonline{vasconcelosraposo2016}, com adaptações lexicais mínimas para o português brasileiro.
\end{itemize}
